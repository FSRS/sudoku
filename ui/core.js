...
let puzzleSelectionRequestId = 0;
...
function updateSolverToggleButton() {
  ...
  if (isViewAllTechniquesMode && vatSelectedHint) {
    ...
    btn.textContent = isMobile
      ? t("ui_exit_solver_btn_mobile")
      : t("ui_exit_solver_btn");
    btn.className =
    ...
    btn.textContent = isMobile
      ? t("ui_solver_mode_btn_mobile")
      : t("ui_enter_solver_mode_btn");
    btn.className =
      "w-full inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-orange-500 hover:bg-orange-600 transition-colors";
...
function updateButtonLabels() {
  ...
  if (currentMode === "pencil") {
    modeToggleButton.textContent = isMobile
      ? t("ui_pencil_mode_btn_mobile")
      : t("ui_pencil_mode_btn");
    modeToggleButton.dataset.tooltip = t("ui_pencil_mode_tooltip");
  } else {
    modeToggleButton.textContent = isMobile
      ? t("ui_number_mode_btn_mobile")
      : t("ui_number_mode_btn");
  ...
  if (currentMode === "draw") {
    const label =
      drawSubMode === "solid"
        ? t("ui_draw_solid_mode_label")
        : t("ui_draw_dash_mode_label");
  ...
  if (currentMode === "color") {
    if (coloringSubMode === "cell") {
      colorButton.textContent = isMobile
        ? t("ui_color_cell_mode_btn_mobile")
        : t("ui_color_cell_mode_btn");
      colorButton.dataset.tooltip = t("ui_color_cell_mode_tooltip");
    } else if (coloringSubMode === "candidate") {
      colorButton.textContent = isMobile
        ? t("ui_color_cand_mode_btn_mobile")
        : t("ui_color_cand_mode_btn");
...
function addSudokuCoachLink(puzzleString) {
  ...
  btn.dataset.tooltip = isMobile
    ? t("ui_solver_toggle_btn_mobile")
    : t("ui_solver_toggle_btn");
...
function handleDrawClick(r, c, n, overrideStyle = null, overrideColor = null) {
  ...
  const activeColor =
    drawingState.color || selectedColor || lineColorPalette[0];
...
function updatePreview() {
  ...
  const activeStyle = drawingState.style || drawSubMode;
  const color =
    drawingState.color || selectedColor || lineColorPalette[0] || "black";
...
function updateLamp(color, { record = true, level = null } = {}) {
  ...
  if (baseLabels[color]) {
    ...
    if (level !== null && color !== "gray") {
      ...
      desc = isCustom
        ? t("ui_custom_difficulty_level", level)
        : t("ui_difficulty_level_value", level);
    }
...
function setupEventListeners() {
  ...
  shareBtn.addEventListener("click", (e) => {
    ...
    document.getElementById("share-modal-desc").textContent = t(
      "ui_share_target_prompt",
    );
  ...
  shareCurrentBtn.addEventListener("click", () => {
    ...
    shareStep2.classList.add("flex");
    document.getElementById("share-modal-desc").textContent = t(
      "ui_share_progress_format_label",
    );
  });
...
function handleModeChange(e, reverse = false) {
  ...
  if (currentMode === "concrete") {
    tip = isMobile
      ? t("ui_highlight_number_tip_mobile")
      : t("ui_msg_121_desktop");
  } else if (currentMode === "pencil") {
    ...
    if (coloringSubMode === "cell") {
      ...
      tip = isMobile
        ? t("ui_cand_color_select_tip_mobile")
        : t("ui_cand_color_preview_tip");
    } else if (coloringSubMode === "circle") {
...
async function loadSavedDailyPuzzle(date, level) {
  ...
  const loaded = await loadPuzzle(savedGame.puzzle, puzzleData);
...
async function findAndLoadSelectedPuzzle() {
  ...
  try {
    ...
    if (puzzle) {
      ...
      const loaded = await loadPuzzle(decompressedPuzzle, puzzleData);
      if (!loaded) return;
      if (shouldFallbackLevel11) {
    ...
    if (requestId !== puzzleSelectionRequestId) return;
    console.error(err);
    const loadedSavedPuzzle = await loadSavedDailyPuzzle(
      selectedDateInt,
      selectedLevel,
    );
    if (requestId !== puzzleSelectionRequestId) return;
    if (loadedSavedPuzzle) {
      if (shouldFallbackLevel11) {
        showMessage(t("ui_msg_level_11_fallback"), "orange");
      }
      return;
...
function autoPencil(skipConfirm = false) {
  ...
  const isMobile = window.innerWidth <= 550;
  const tip = isMobile
    ? t("ui_bivalue_highlight_tip_mobile")
    : t("ui_bivalue_highlight_tip");
...
async function loadPuzzle(puzzleString, puzzleData = null) {
  ...
  const libraryState = parsedInput.libraryState || null;
  ...
  if (puzzleData) {
    ...
    puzzleScoreEl.textContent =
      customScoreEvaluated > 0
        ? `~${customScoreEvaluated}${customPrefsStar}`
        : "";
...
function getHistoryEntryDescription(entry) {
  ...
  const lineDesc = entry.linePatch
    ...
    : null;
  if (actionDesc === t("ui_undo_no_visible_changes") && lineDesc)
    return lineDesc;
...
const getUniquenessTechniques = () =>
  SudokuTechniqueCatalog.getUniquenessNames(t);
const getMandatoryTechniques = () =>
  SudokuTechniqueCatalog.getMandatoryNames(t);
...
function encodeBoardState() {
  return encodeSharedBoardState(boardState);
}
...
async function handleUrlParameters() {
  ...
  if (stateStr) {
    ...
    const userCells = decodedState.userCells;
    dateSelect.value = "";
    levelSelect.value = "";
    ...
    userCells.forEach((uc) => {
      if (uc.type === "user") {
        boardState[Math.floor(uc.index / 9)][uc.index % 9].value = uc.value;
      } else if (uc.type === "candidates") {
        uc.values.forEach((value) =>
          boardState[Math.floor(uc.index / 9)][uc.index % 9].pencils.add(value),
        );
      }
    });
