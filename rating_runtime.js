...
(function (root, makeRuntime) {
  if (typeof module === "object" && module.exports) {
    ...
    root.createRating = (options) =>
      makeRuntime(root.createRatingNative, options);
  }
})(globalThis, async function (createNative, options) {
  const native = await createNative(options || {});
  const modeError =
    "ERROR,java.lang.IllegalArgumentException,mode must be 0 (current) or 1 (SE 1.2.1)";
  const stateError =
    "ERROR,java.lang.IllegalArgumentException,invalid rating state";
  ...
  function encodeState(state) {
    ...
    for (let cell = 0; cell < 81; cell++) {
      ...
      masks += (state.charCodeAt(81 + cell) & 1022)
        .toString(16)
        .padStart(3, "0");
    }
    return values + ":" + masks;
